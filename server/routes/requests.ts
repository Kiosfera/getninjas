    ...
    proposals: [
      {
        ...
        message:
          "Olá! Posso te ajudar com a instalação do chuveiro. Tenho 10 anos de experiência e posso fazer hoje mesmo.",
        price: 200,
...
export const handleCreateProposal: RequestHandler = (req, res) => {
  ...
  const professionalId =
    req.headers.authorization?.replace("Bearer ", "") || "prof_1"; // Mock auth
...
export const handleGetNearbyRequests: RequestHandler = (req, res) => {
  ...
  const nearbyRequests = requests.filter(
    (request) => request.status === "open" && request.location.coordinates,
  );
